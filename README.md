...
where $\mathbf{y}$ is the phenotype vector, $\mathbf{W_{⍺h}}$ is our multi-allelic matrix, $\boldsymbol{\beta_{⍺h}}$ contains all allele effects, and $\mathbf{e}$ is residual error. The key difference is that $\boldsymbol{\beta}$ now has thousands or tens of thousands of parameters instead of hundreds of thousands of SNPs, but each parameter carries more biological information.
