...
where $\mathbf{y}$ is the phenotype vector, $\mathbf{W}$ is our multi-allelic matrix, $\boldsymbol{\beta}$ contains all allele effects, and $\mathbf{e}$ is residual error—the key difference is that $\boldsymbol{\beta}$ now has thousands or tens of thousands of parameters instead of hundreds of thousands of SNPs, but each parameter carries more biological information.
