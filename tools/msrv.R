...
# ~/.cargo/bin is the default install location for rustup-managed toolchains
# and is not always on PATH in non-interactive R sessions (e.g. R CMD INSTALL)
new_path <- paste0(
  Sys.getenv("PATH"),
  ":",
...
if (!is.na(msrv)) {
  # compareVersion returns 1 when first arg is newer than second;
  # here msrv > current means the installed version is too old
