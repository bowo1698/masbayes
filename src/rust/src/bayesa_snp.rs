...
impl BayesASNPRunner {
    pub fn new(
        ...
        let sigma2_j = Array1::<f64>::from_elem(n_alleles, s_squared);
    ...
    pub fn run(&mut self) -> BayesAResults {
        ...
        if self.verbose {
            eprintln!("[Fold {}] BayesA-SNP MCMC | n={} p={} n_iter={} n_burn={} binary={}",
        ...
        for iter in 0..self.n_iter {
            ...
            // Per marker: sample σ²_j first, then β_i conditional on it.
            for i in 0..self.n_alleles {
                let xx = self.wtw_diag[i];
                if xx < 1e-30 { continue; }
                let old_gi = self.beta[i];

                let shape_j = (self.nu + 1.0) / 2.0;
                let scale_j = (self.nu * self.s_squared + old_gi.powi(2)) / 2.0;
                self.sigma2_j[i] = rinvgamma(&mut self.rng, shape_j, scale_j);
                ...
                rhs += xx * old_gi;
                let v = xx + self.sigma2_e / self.sigma2_j[i];
        ...
        if self.verbose {
            eprintln!("[Fold {}] BayesA-SNP done | σ²_g = {:.6} | h² = {:.4}",
...
mod tests {
    ...
    fn snp_kernel_smoke_gaussian() {
        ...
        let wtw_diag: Vec<f64> = (0..p)
            ...
            .collect();
        let mut runner = BayesASNPRunner::new(
            w, y, wtw_diag, None,
            4.0, 0.1, 1.0,
            -1.0, 0.0,
            200, 100, 5,
    ...
    fn snp_kernel_smoke_binary() {
        ...
        assert_eq!(r.z_hat.as_ref().unwrap().len(), n);
    }
}
